...
def propagate(w, b, X, Y):
    ...
    z = np.dot(w.T, X)
    A = sigmoid(z + b)  # compute activation
    cost = (-1.0/m) * np.sum(Y * np.log(A) + (1 - Y) * np.log(1 - A))

    # BACKWARD PROPAGATION (TO FIND GRAD)
    a_minus_y = A - Y
    dw = (1.0/m) * np.dot(X, a_minus_y.T)
    db = np.mean(a_minus_y)
...
num_iterations = 2000
learning_rate = 0.005
d = model(train_set_x, train_set_y, test_set_x, test_set_y, num_iterations=num_iterations, learning_rate=learning_rate,
          print_cost = True)

costs = np.squeeze(d['costs'])
plotting_service.learning_curve(costs, learning_rate)
